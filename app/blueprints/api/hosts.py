...
from app.models import Alert, Host, IPRegistry, LogArchive, LogSource
...
def get_windows_info(host_id):
    ...
    if host.os_type != "WINDOWS":
        return jsonify({"error": "Wrong OS"}), 400
    try:
...
def add_ip():
    ...
    if not data:
        return jsonify({"error": "brak danych"}), 400
    in_ip = data.get("ip_address")
    in_status = data.get("status")
    try:
        ip_entry = IPRegistry(ip_address=in_ip, status=in_status)
        db.session.add(ip_entry)
        db.session.commit()
        return jsonify({"message": f"dodano {in_ip}"}), 201
    except Exception as e:
        db.session.rollback()
...
def update_ip(ip_id):
    ...
    try:
        ip_entry.ip_address = data.get("ip_address")
        ip_entry.status = data.get("status")
        db.session.commit()
    except Exception as e:
        db.session.rollback()
        return jsonify({"error": str(e)}), 400
    return jsonify({"message": "zmieniono IP"}), 200
...
def delete_ip(ip_id):
    ...
    try:
        ...
        db.session.commit()
        return jsonify({"message": f"deleted ip record"}), 200
    except Exception as e:
        db.session.rollback()
        return jsonify({"error": str(e)}), 400
...
def get_recent_alerts():
    ...
    return jsonify([alert.to_dict() for alert in alert_query])
