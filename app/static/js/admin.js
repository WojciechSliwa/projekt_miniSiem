...
export async function initAdmin() {
  ...
  if (document.getElementById("saveHostBtn")) {
    document
      .getElementById("saveHostBtn")
      .addEventListener("click", handleSaveHost);
  }
  ...
  if (document.getElementById("saveIPBtn")) {
    document
      .getElementById("saveIPBtn")
      .addEventListener("click", handleSaveIP);
  }
...
async function refreshIPs() {
  ...
  try {
    ...
    if (ips.length === 0)
      createEl("div", ["p-2", "text-muted", "small"], "Pusto.", ipContainer);
    ips.forEach(renderIPRow);
  } catch (e) {
    console.error("Błąd IP:", e);
  }
}
