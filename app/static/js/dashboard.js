  ...
  try {
    ...
    if (hosts.length === 0) {
      createEl(
        ...
        "Brak hostów. Skonfiguruj system w panelu admina.",
        hostsContainer,
      );
      return;
    }
...
function renderDashboardRow(host) {
  const item = createEl(
    "div",
    ["list-group-item", "py-3", "border-bottom"],
    "",
    hostsContainer,
  );
  const row = createEl(
    "div",
    ["row", "align-items-center", "flex-nowrap", "g-0"],
    "",
    item,
  );

  // KOLUMNA 1: INFO
  const colInfo = createEl(
    "div",
    ["col-4", "d-flex", "align-items-center", "overflow-hidden"],
    "",
    row,
  );
  ...
  createEl(
    "div",
    ["text-muted", "small", "text-center", "fst-italic"],
    "Kliknij Status...",
    colStatus,
  );
  ...
  checkBtn.addEventListener("click", () =>
    handleCheckStatusFancy(host, colStatus, checkBtn),
  );

  const logsBtn = createEl(
    "button",
...
async function handleCheckStatusFancy(host, container, btn) {
  ...
  try {
    ...
    createEl(
      "div",
      ["text-danger", "small", "fw-bold", "text-center"],
      "Błąd",
      container,
    );
    btn.innerHTML = "Ponów";
...
function addBadge(parent, label, value, colorClass) {
  const box = createEl(
    "div",
    ["text-center", "border", "rounded", "bg-light", "py-1"],
    "",
    parent,
  );
  box.style.width = "24%";
  ...
  const val = createEl(
    "div",
    ["fw-bold", "text-nowrap", colorClass],
    value || "?",
    box,
  );
  ...
  try {
    ...
    if (alerts.length === 0) {
      ...
      const cell = createEl(
        ...
        "Brak alertów (lub brak połączenia z API).",
        row,
      );
      cell.colSpan = 6;
      return;
